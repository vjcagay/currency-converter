...
app.get("/history", (req, res) => {
  ...
  if (fromUpperCase !== toUpperCase) {
    const startRate = processedData[processedData.length - 1]?.rate || 0;
    const endRate = processedData[0]?.rate || 0;
    // Round to the nearest thousandth decimal
    // 100,000 is just 100 (for %) * 1000 (for dividing to 1000)
    gainLossPercentage = startRate && endRate ? Math.round(((startRate - endRate) / endRate) * 1e5) / 1e3 : 0;
  }
